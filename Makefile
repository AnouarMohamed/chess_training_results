...
.PHONY: dev up down logs test tidy fmt lint sqlc migrate-up migrate-down build run print-env-template
...
run:
	DATABASE_URL="$(DATABASE_URL)" JWT_SECRET="$(JWT_SECRET)" JWT_TTL_MIN="$(JWT_TTL_MIN)" HTTP_ADDR="$(HTTP_ADDR)" APP_ENV="$(APP_ENV)" go run ./cmd/api
